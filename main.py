...
def get_driver_name_from_code(driver_code):
        ...
        for i in data:
            ...
            for i in stripped:
                code_to_name[stripped[1]] = stripped[2]
...
def display_average_and_fast(driver_time_dict):
    for k, v in driver_time_dict.items():
        full_name = get_driver_name_from_code(k)
        print(f"FOR {full_name}/{k}")
...
def display_results():
    ...
    full_name = get_driver_name_from_code(code)
    ...
    print(f"THE FASTEST DRIVER WAS {full_name}\n")
